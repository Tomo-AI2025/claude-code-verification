...
- **arxiv 2603.20847(FSE '26)** は Claude Code の 3,800 件の実問題を分析し、**67% が機能バグ**、うち 37.3% が API/統合のミスマッチに起因すると報告しています。これは Part 1 の「サイレント・フェイラー」と同じカテゴリです。
- **arxiv 2604.08906** は agentic LLM フレームワーク全般で silent semantic error を記録しています。本稿の Pattern #7 はその具体的かつ測定可能なインスタンスに相当します。
- **GitHub Issue #19739(anthropics/claude-code)** は体系的な失敗パターンを蓄積するコミュニティスレッドで、本稿の新パターンとも一部重なります。
- **BSWEN(2026年3月)** は「phantom API hallucination」を定性的に記述しましたが、頻度は示していません。本稿はその空白を埋めます。
- **GitHub Issue #19117(anthropics/claude-code)** は Anthropic 公式の telemetry 設定ドキュメント内の用語曖昧性を報告しています。Pattern #9 がローカルな癖ではなく構造的問題であることの証左です。
...
1. **arxiv 2603.20847** — *A Systematic Study of Bug Patterns in Claude Code*(FSE '26)。3,800 件の issue 分析、うち 67% が機能バグ。
2. **arxiv 2604.08906** — agentic LLM フレームワーク全般における silent semantic error。
3. **GitHub Issue #19739(anthropics/claude-code)** — 体系的失敗パターンを蓄積するコミュニティスレッド。
4. **BSWEN(2026年3月)** — phantom API hallucination の定性記述。
5. **GitHub Issue #19117(anthropics/claude-code)** — Anthropic 公式ドキュメントにおける telemetry の用語曖昧性。
6. **cclint**(carlrannaberg) — Claude Code 設定 linter。
7. **opslane/verify** — ブラウザベースの UI テストランナー。
8. **claude-code-quality-hook**(dhofheinz) — Claude Code リポジトリ向けの pre-commit lint 修正ツール。
9. **spec-kit**(GitHub) — 仕様記述ワークフロー。
